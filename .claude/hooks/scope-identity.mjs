...
function workOrderBlock(zoneDocs) {
  return [
    ...
    ``,
    `1. **ТЗ читаешь в трекере** (Windshift MCP, боевой). Не в этой переписке и не по файлам`,
    `   репозитория. Трекер не отвечает — скажи об этом вслух, не переходи молча на работу по памяти.`,
    `2. **Доку ${zoneDocs} читаешь целиком до первой правки** — \`README.md\` (устройство),`,
    `   \`FAQ.md\` (почему так), \`ROADMAP.yaml\` (что запланировано). Это рабочий контекст, а не`,
    `   украшение: решения, принятые без него, регулярно противоречат уже принятым.`,
    ...
    ``,
    `## Четыре самых дорогих ошибки здесь`,
    ``,

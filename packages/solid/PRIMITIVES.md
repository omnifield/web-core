# 🔍 Solid Primitives — РЕСЕРЧ рынка, не состав поставки

> ## ⚠️ Это НЕ список того, что даёт `@web-core/solid`
>
> Здесь лежит обзор **чужого** каталога — что вообще существует в экосистеме Solid и что мы
> **могли бы** вытянуть, когда появится повод. Ни одна строка ниже не означает, что примитив
> доступен из нашего пакета.
>
> **Из всего этого списка у нас подключён ровно один: `keyed`** (подпуть
> `@web-core/solid/keyed`). Взятые помечены ✅ — больше меток нет.
>
> Что пакет отдаёт на самом деле — [`README.md`](./README.md), раздел «Анатомия». Как этим
> пользоваться — [`EXAMPLES.md`](./EXAMPLES.md).
>
> **Нужен ещё один примитив — это заявка архитектору, а не `pnpm add` в своей зоне:** зависимость
> в поставку решает он, владелец зоны только предлагает и обосновывает.
...
- **keyed** — ✅ **взят**, живёт подпутём `@web-core/solid/keyed` (см. README.md). Рендер списков с явными ключами. [docs](https://primitives.solidjs.community/package/keyed/) · [src](https://github.com/solidjs-community/solid-primitives/tree/main/packages/keyed)

// Схемы для обёрток из `./index.ts` — зеркалят TS-типы движка через `z.ZodType<T>`, соответствие
// проверяет компилятор. Паспорт и lookup сюда не входят — разбор в FAQ.md.

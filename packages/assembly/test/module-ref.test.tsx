// Проба узла-ссылки на модуль со стороны отрисовки: подстановка чужого дерева на месте узла,
// живость (правка исходного модуля доезжает во ВСЕ места, где он вставлен) и гвард круга.
//
// Реестр — свои, синтетические компоненты, не кит: предмет пробы механика подстановки, а не
// разметка конкретного компонента. Источник модулей подан тем же третьим входом
// `createRegistry`, каким его подаёт приложение, и читает СИГНАЛ — именно так живость и
// получается: источник читается внутри мемо отрисовки, значит его зависимости отслеживаются.

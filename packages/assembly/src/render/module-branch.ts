...
/**
 * Дерево, которое подставляется на месте узла-ссылки. Источник модулей читается ВНУТРИ мемо —
 * отсюда живость: сигнал приложения, прочитанный источником, становится зависимостью этого мемо,
 * и правка исходного модуля доезжает до всех мест, где он вставлен, сама.
 *
 * Гвард по стеку имён — вторая линия, не первая: цикл ловится отказом на вставке
 * (`module-cycle`, `engine/edits.ts`), но дерево может прийти и мимо правок — из файла, из сети,
 * от старого редактора. Без гварда такой цикл кончается не сообщением, а бесконечной рекурсией
 * отрисовки.
 */

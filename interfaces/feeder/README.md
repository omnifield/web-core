...
- `entities/openapi` — наш формат API: `SchemaDocument { endpoints, defs }`, распознавание
  Swagger 2.0 (`swagger2Template` поверх `@web-core/generators/mapping`), простановка айди
  ручкам (`identify`), JSON Schema → zod (`schemaNodeToZod`), группировка по тегам, правки
  состава, примитивы вызова;
...
(сегодня один, Swagger 2.0, совпадение жёсткое), выдаёт каждой ручке айди и отдаёт
`SchemaDocument`. Нераспознанный документ — исключение, экран показывает его текстом.

↔️ **Документ.** `SchemaDocument { endpoints, defs }`; ручка — `{ id, method, url, tag?, params }`,
где `params[].schema` — JSON Schema, а `defs` держит общие типы (`$ref` остаются ссылками, циклы не
...
↔️ **Айди ручки.** Уникальный, выдаётся при разборе и ни от чего не производен. Им ручка
опознаётся в списках и в правках состава: метод, урл и тег юзер правит, и тождество на них не
держится. Документ без айди у ручки схемой не признаётся (`asSchemaDocument` вернёт `undefined`) —
единственное место, где айди проставляются, это входной слой.
...
🧪 Vitest + jsdom. **115 тестов, все зелёные:**

- `entities/openapi` — распознавание petstore, айди ручкам при разборе (уникальны, не производны
  от содержимого), JSON Schema → zod (вложенные `$ref`, циклы через `z.lazy`), дескриптор → ручка,
  группировка по тегам, правки состава, живучесть узлов состава при правке урла;
...
собранным из полей: метод, урл и тег правит юзер, и ключ на них разъезжается с сущностью. Разбор —
`FAQ.md`, «Списки и тождество узла».

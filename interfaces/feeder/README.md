...
Ключевое решение, на котором стоит вся механика: **привязка и адаптер — одна запись, а не две**.
`Binding` несёт и «что дёрнуть» (`source`), и «где записи» (`root`), и «как запись ложится на вход
компонента» (`rules`). Адаптер без привязки некуда деть, привязка без адаптера не кормит — разбор в
[FAQ.md](./FAQ.md), раздел «Привязка компонента к ручке».

⚠️ **Статус на 2026-09-18.** Пакет пережил снос и пересборку: прежние публичные виджеты
(`OpenapiEditor`, `OpenapiList`, `Mapping a/b`, `Tree` как экспорт) больше не существуют. Всё, что
описано ниже, соответствует коду; исторические решения в FAQ.md помечены там, где описанного API
уже нет. Мод «файл» (источник неизвестной формы) не реализован.

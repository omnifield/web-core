...
import { usePreviewStore } from "../../model";

/**
 * Выбор пресета данных для всего показа.
 *
 * В контрол уходят только имя и подпись — тело пресета через него НЕ течёт. Тело живёт в кэше
 * запросов, и всё, что показ о нём помнит, — выбранное имя (разбор у типа `Feed` в
...
export function FeedPreset() {
  ...
  createEffect(() => {
    ...
    if (store.selectors.previewFeed()?.kind === "manual") return;

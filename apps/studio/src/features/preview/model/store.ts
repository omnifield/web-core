 ...
 * `model/preview.ts`, там же, где стор встречается с сущностью.
 *
 * Ручные данные — другое дело: у них нет другого хозяина, показ их и сочинил, поэтому они лежат
 * здесь целиком.
 */
export type Feed =
...
export const previewStoreOf = createActionStoreFamily<
  ...
  },
  {
    ...
    feed(state: PreviewState, cell: Cell): Feed | undefined;
    /** Корм всего показа — то, чем кормят панели `ui/feed`. Отдельный селектор, а не `feed` с
     *  необязательной ячейкой: параметризованный селектор узнают по arity сигнатуры, а
     *  необязательный аргумент в неё не считается. */
    previewFeed(state: PreviewState): Feed | undefined;

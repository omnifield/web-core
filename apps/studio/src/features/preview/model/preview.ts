...
import { previewStoreOf, type Feed } from "./store";
...
export function usePreview() {
   ...
   *  там, показ держит только выбор. Пресет мог уехать (сменился компонент, кэш ещё едет) —
  ...
  return {
    ...
    /** Чем накормлена ячейка и чем накормлен показ целиком — уже данными, а не ссылкой. */
    feedDataOf: (cell: Cell) => dataOf(store.selectors.feed(cell)),
    previewFeedData: () => dataOf(store.selectors.previewFeed()),

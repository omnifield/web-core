...
export const Route = createFileRoute("/_workspace/showcase/component/{-$name}")({
  loader: ({ params }) => {
    if (params.name === undefined) throw notFound();
  },
  onEnter: ({ params }) => selectComponent(params.name),
  onStay: ({ params }) => selectComponent(params.name),

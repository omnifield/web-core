// Разрешение «ячейка → показанный элемент» (`features/preview/lib/axes.ts`) и адресация
...
// — показу. Поэтому арифметика здесь проверяется чистой, без стора, а стор — без списков.
//
...
import {
  ...
  secondaryIndexIn,
  variantIn,
} from "#/features/preview/lib/axes";
...
describe("стор адресует хранимый выбор, ничего не зная о списках", () => {
  it("ячейка и группа — разные адреса, даже если тег назван как позиция", () => {
    const store = previewStoreOf("ключ-не-сталкивается");
  ...
  it("смена оси не переносит выбор в чужой список и возвращает свой обратно", () => {
    const store = previewStoreOf("смена-оси");
  ...
  it("выбор на обёртке доезжает до всех её слайдов и не трогает соседнюю", () => {
    const store = previewStoreOf("обёртка-адресует-группу");
    store.actions.setLayoutMode("matrix");
  ...
  it("групповой выбор виден контролу обёртки без единой ячейки на руках", () => {
    const store = previewStoreOf("контрол-без-ячейки");

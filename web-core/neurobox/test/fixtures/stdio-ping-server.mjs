// Реальный, независимый от @web-core/neurobox stdio-сервер — только для теста stdioPeer в
// mcp.test.ts. Не через createMCPClient/наш пакет нарочно: фикстура должна быть запускаема голым
// `node` как отдельный процесс, не завязанным на то, собран ли сейчас dist пакета.
...
// (PATH/HOME/...), не весь process.env запускающего — см. mcp.test.ts, env passthrough.
server.registerTool(
  "env-probe",

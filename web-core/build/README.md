...
независимых дев-плагина, каждый в своём файле, `nested-cjs.ts` — обход дерева зависимостей для
предварительной оптимизации. `src/vitest/index.ts` — самодостаточен, один файл.
...
| Дев-режим — CommonJS из глубины пакета с сырой разметкой пребандлен | в `optimizeDeps.include` стоит цепочка `<сосед> > … > <пакет>` | `/vite`, `nested-cjs.ts` |
| Сборка — ничего не подменяется | оба дев-плагина `apply: "serve"`, в билде не участвуют; цепочки считаются только на `serve` | `/vite` |
...
🧩 Рядом с ним — `test/fixture-dep/`: пакет-сосед стенда, который тянет зависимость с сырой
разметкой, а та в глубине держит CommonJS. Отдельный пакет нужен потому, что класс живёт на
границе «сосед по воркспейсу → чужой пакет», и внутри одного проекта его не собрать. Проба зоны
(`test/nested-cjs.test.ts`) поднимает на этом стенде настоящий дев-сервер и смотрит, что уехало
потребителю.
...
| Сканер зависимостей не спотыкается о разметку | стенд держит файл, где `import.meta.glob` и разметка лежат вместе (`src/globbed.tsx`) — дев-сервер на нём | скан проходит, предварительная оптимизация не пропускается |
| CommonJS из глубины пакета с сырой разметкой доезжает разобранным | проба зоны на стенде с соседом `test/fixture-dep`: дев-сервер, запрос главного модуля, разбор того, что попало в предварительную оптимизацию | цепочка названа, чанк собран; без правки та же проба красная |
| Страница компонента с разбором markdown в реальном приложении | дев-сервер приложения-витрины на этой оснастке, вкладка консоли | ни одной ошибки; прод-сборка того же приложения зелёная |

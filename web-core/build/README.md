...
разрешения тестов. Девять точек поверхности закрывают весь цикл: конфиг приложения и конфиг
библиотеки (`/vite`), пресет тестов (`/vitest`) и API тест-раннера (`/test`), два профиля типов ⚙️
— для фронтенда и для сервера без Vite (`/tsconfig`, `/tsconfig-node`) — типы клиентской среды
(`/client`), три бинарника (`web-core-node`, `web-core-vite`, `web-core-vitest`) и рантайм-ридер
env для приложений (`/env`).

📦 Инструменты 🔧 едут ВНУТРИ пакета, а не требуются от потребителя: бандлер, тест-раннер и
DOM-эмулятор — обычные зависимости этой зоны, наружу они выходят бинарниками и типами. Поэтому в
манифесте приложения их имён нет вовсе — разбор, почему так, в [`FAQ.md`](./FAQ.md). Пять из шести только конфигурируют сборку потребителя и не
...
| API тест-раннера | `@web-core/build/test` | весь состав раннера (`describe`, `it`, `expect`, `vi`, …) — импортируется в файлах проб |
...
| Типы клиентской среды | `@web-core/build/client` | ссылку на типы бандлера — имя для `types` в `tsconfig.json` потребителя |
...
| Бандлер | бинарник `web-core-vite` | CLI бандлера, аргументы как есть (`web-core-vite build`, `web-core-vite preview`) |
| Тест-раннер | бинарник `web-core-vitest` | CLI раннера, аргументы как есть (`web-core-vitest run`) |
...
```jsonc
// package.json потребителя — команды приходят бинарниками зоны, не своей зависимостью
"scripts": {
  "dev": "web-core-vite",
  "build": "web-core-vite build",
  "test": "web-core-vitest run"
}
```

```tsx
// файл пробы потребителя — API раннера тоже приходит отсюда
import { describe, expect, it } from "@web-core/build/test";

describe("счётчик", () => {
  it("начинается с нуля", () => expect(0).toBe(0));
});
```
...
🧪 У зоны есть стенд-потребитель — `test/fixture/`: настоящее приложение, у которого в манифесте
только пакеты базы. Он и есть проверка поверхности: собственная сборка зоны такого класса ошибок
не видит вовсе. Прогоны стенда — `typecheck`, `test`, `build`, плюс живой дев-сервер.

⚠️ Прежний набор проб зоны шире (разбор тарбола, HTTP к дев-серверу отдельными сценариями) и
восстановлен не целиком — остаток в `ROADMAP.yaml`, `id: rebuild-test-suite`. Ниже — что
фактически проверено ✅ на стенде и вручную.
...
| Соседи по воркспейсу видны исходником и тестам | то же приложение, временный экспорт в `web-core/trace/src/` (нет в `dist`), тест через `@web-core/build/vitest` | тест увидел экспорт без пересборки соседа |
...
| Потребитель без единого стороннего имени в манифесте поднимается целиком | стенд `test/fixture`: `typecheck`, `test`, `build`, дев-сервер | все четыре зелёные; корень дев-сервера — `200` |
| API раннера приходит подпутём, а не своей зависимостью | проба стенда импортирует `describe`/`it`/`expect` из `@web-core/build/test` | настоящий прогон, 2 пробы |
| Вендор разрешается, хотя потребитель его не объявляет | тот же стенд: рендер Solid-компонента в пробе плюс модуль обвязки горячей замены на дев-сервере | проба зелёная; обвязка отдаёт `200`, её импорт вендора разрешён |
| В коде потребителя нет прямых импортов вендора | трансформ `src/counter.tsx` на дев-сервере | только фасад и служебные модули сборщика |

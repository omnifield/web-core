...
разрешения тестов. Восемь точек поверхности закрывают весь цикл: конфиг приложения и конфиг
библиотеки (`/vite`), пресет тестов (`/vitest`), два профиля типов ⚙️ — для фронтенда и для
сервера без Vite (`/tsconfig`, `/tsconfig-node`) — типы клиентской среды (`/client`), три
бинарника (`web-core-node`, `web-core-vite`, `web-core-vitest`) и рантайм-ридер env для
приложений (`/env`).

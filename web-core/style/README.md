...
🗺️ По образцу `web-core/store`: `src/index.ts` — тонкая поверхность (`export * from "./engine/index.js"`), сама логика и сам перечень поверхности — в `src/engine/`. Дальше разрез
...
| `DERIVED_SCALES` (`column`) | имя ступени равно её числу знаков, ряд не переходит потолок нормы (80 знаков) и зажимает идеал измерения (66) ступенями с двух сторон | `test/column-measure.test.ts` |

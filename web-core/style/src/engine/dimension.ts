...
export const DERIVED_SCALES: readonly DerivedScale[] = [
  ...
  },
  {
    ...
    snap: true,
    basis:
      "читаемая колонка: семя — средняя ширина одного знака при базовом кегле, имя ступени равно числу знаков (`--column-40` — сорок знаков). Из кратности интервалу ширина не выводится: ритм отступов к длине строки отношения не имеет. Ряд покрывает канон измерения (45–75 знаков, идеал 66) и не доходит до потолка в 80 знаков (WCAG 2.2, 1.4.8 Visual Presentation, AAA); после умножения плотностью значение возвращается на сетку 0.25rem",
    steps: [
      ...
      { name: "column-64", factor: 64 },
      { name: "column-72", factor: 72 },
    ],
  },
  {
    seed: "control-height",

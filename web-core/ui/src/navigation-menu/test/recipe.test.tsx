...
import { editorInfo } from "../playground";
...
const outfit: Outfit = { name: "navigation-menu-proof", palette: palette.name, forms: [form.name] };
const assembled = bound.assemble(outfit, { palettes: [palette], forms: [form] });

describe("navigation-menu proof recipe — the passport dressed whole", () => {
  ...
  it("generates real CSS, not an empty rule set", () => {
    ...
    expect(css).toContain('[data-scope="navigation-menu"]');
  });

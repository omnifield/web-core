...
describe("the floating panel scrolls inside its own content, not the document", () => {
  it("puts flex column on the positioner and a shrinkable, scrollable content — on nodes that really exist", async () => {
        ...
        <Popover defaultOpen>
          <PopoverPositioner>
            <PopoverContent>
              ...
              <PopoverDescription>{"Очень длинное описание. ".repeat(200)}</PopoverDescription>
            </PopoverContent>
          </PopoverPositioner>
        </Popover>
      ),
    ...
    const positionerSelector = '[data-scope="popover"][data-part="positioner"]';
    const contentSelector = '[data-scope="popover"][data-part="content"]';

...
сборка (`web-core/assembly`), и наполнение сборки данными не должно решаться заново в каждом
...
`web-core/assembly`: пакет не решает, что именно должно получиться, только чем это сделать.

🧬 Паспорт формы данных — НЕ поле `ComponentPassport` (`web-core/skin`), а свой, отдельный реестр,
которым владеет этот пакет и только он: скин не зависит от данных, данные не зависят от скина
(решено user, 2026-08-29). `web-core/io` не импортирует `web-core/skin` и не будет.
...
своей логики — тем же приёмом, что `web-core/store`/`web-core/assembly`/`web-core/router`.
